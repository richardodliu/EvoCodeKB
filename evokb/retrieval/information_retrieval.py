...
from ..fingerprint.comment_generator import CommentFingerprintGenerator
...
class InformationRetrieval:
    ...
    def __init__(self, database: Database,
                 comment_fp_generator: CommentFingerprintGenerator):
        self.database = database
        self.comment_fp_generator = comment_fp_generator
    ...
    def retrieve(self,
        ...
        """
        # 1. 生成输入文本的注释指纹
        origin_cur_refer_trees = self.comment_fp_generator.generate(input_text)
        if not origin_cur_refer_trees:
            print("警告: 无法生成输入文本的注释指纹")
            return []

        cur_refer_trees = origin_cur_refer_trees.copy()

        # 2. 从数据库获取候选代码的轻量指纹信息
        db_candidates = self.database.query_fingerprints(language=language,
        ...
        for cand in db_candidates:
            if cand['comment_fingerprint']:
                try:
                    fp_tree = json.loads(cand['comment_fingerprint'])
                    candidate_trees[cand['id']] = fp_tree
                    valid_candidates.append(cand)
                except Exception as e:
                    print(f"警告: 解析注释指纹失败 {cand['relative_path']}: {e}")

        if not valid_candidates:
            print("警告: 没有有效的候选代码（缺少注释指纹）")
            return []

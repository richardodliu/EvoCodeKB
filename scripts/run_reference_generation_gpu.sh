...
OUTPUT_DIR=""
OUTPUT_JSONL="${REPO_ROOT}/benchmark/output_n5_max100_shots10_information_2gram_gen.jsonl"
MODEL_PATH="/volume/pt-train/users/rbliu/model/Qwen2.5-Coder-7B"

PROMPT_BUILDER="plus_local16"
STOP_POLICY="first_nonempty_line"
DECODE_POLICY="beam"
BEAM_SIZE=4
REFERENCE_SHOTS=5
MAX_NEW_TOKENS=128
...
usage() {
  cat <<'EOF'
Usage: bash scripts/run_reference_generation_gpu.sh [options]
...
Options:
  ...
  --prompt-builder NAME      Prompt builder (default: plus_local16)
  --stop-policy NAME         Stop policy (default: first_nonempty_line)
  --decode-policy NAME       Decode policy: greedy|beam (default: greedy)
  --beam-size N              Beam size when decode policy is beam (default: 4)
  --reference-shots N        Keep only the first N references per sample (default: 5)
  --max-new-tokens N         Maximum newly generated tokens per sample (default: 128)
  --limit N                  Only process the first N samples before sharding
  --num-gpus N               Number of single-card workers to launch (default: 8)
  --split-mod N              Optional task-id modulo for dev split selection
  --split-rem N              Required remainder when --split-mod is set
...
make_default_run_dir() {
  ...
  run_name="${timestamp}_${PROMPT_BUILDER}_${DECODE_POLICY}_r${REFERENCE_SHOTS}_g${NUM_GPUS}"
  if [[ "${DECODE_POLICY}" = "beam" ]]; then
    run_name="${run_name}_b${BEAM_SIZE}"
  fi
  if (( SPLIT_MOD > 0 )); then
    run_name="${run_name}_split${SPLIT_MOD}r${SPLIT_REM}"
  fi
  if (( LIMIT > 0 )); then
...
meta = {
    ...
    'prompt_mode': 'c_direct_generation',
    'prompt_builder': os.environ['PROMPT_BUILDER'],
    'stop_policy': os.environ['STOP_POLICY'],
    'decode_policy': os.environ['DECODE_POLICY'],
    'beam_size': int(os.environ['BEAM_SIZE']),
    'reference_shots': int(os.environ['REFERENCE_SHOTS']),
    'max_new_tokens': int(os.environ['MAX_NEW_TOKENS']),
    'generation_policy': os.environ['DECODE_POLICY'],
    'split_mod': int(os.environ['SPLIT_MOD']),
    'split_rem': int(os.environ['SPLIT_REM']),
...
if (( MAX_NEW_TOKENS <= 0 )); then
  echo "--max-new-tokens must be > 0" >&2
  exit 1
fi
if (( BEAM_SIZE <= 1 )) && [[ "${DECODE_POLICY}" = 'beam' ]]; then
  echo "--beam-size must be > 1 when --decode-policy beam" >&2
  exit 1
fi
if (( NUM_GPUS <= 0 )); then
...
if (( SPLIT_MOD < 0 )); then
  echo "--split-mod must be >= 0" >&2
  exit 1
fi
if (( SPLIT_MOD > 0 )) && (( SPLIT_REM < 0 || SPLIT_REM >= SPLIT_MOD )); then
  echo "--split-rem must satisfy 0 <= split-rem < split-mod" >&2
  exit 1
fi
...
echo 'Submitting reference generation job via SSH'
...
echo "Stop policy: ${STOP_POLICY}"
echo "Decode policy: ${DECODE_POLICY}"
if [[ "${DECODE_POLICY}" = 'beam' ]]; then
...
if (( SPLIT_MOD > 0 )); then
  echo "Split filter: task_id % ${SPLIT_MOD} == ${SPLIT_REM}"
fi
if (( LIMIT > 0 )); then
  echo "Limit: ${LIMIT}"
fi
...
if [[ "${MODE}" = 'background' ]]; then
  setsid bash "${CONTROLLER_SCRIPT}" \
    ...
    "${NUM_GPUS}" \
    "${SPLIT_MOD}" \
    "${SPLIT_REM}" > "${REMOTE_LOG}" 2>&1 < /dev/null &
  CONTROLLER_PID="$!"
  sleep 1
...
if [[ "${MODE}" = 'background' ]]; then
  ...
  echo 'Watch log: bash scripts/watch_reference_generation_logs.sh'
  echo 'Check status: bash scripts/status_reference_generation_run.sh'
  echo 'Kill run: bash scripts/kill_reference_generation_run.sh'
else
  remote_submit "${MODE}"
  REMOTE_PID_VALUE=''
  REMOTE_PGID_VALUE=''
